...
import { City } from "@/app/utils/definitions";
...
async function getOngkir({ origin, destination, weight, courier }: Ongkir) {
  try {
    ...
    const results = dataJson.rajaongkir.results;

    if (results && results.length > 0) {
      return results[0].costs[0].cost[0].value; // Menyimpan nilai ongkos kirim dari hasil pertama
    }
  } catch (error) {
    console.log("Error post data ongkir", error);
    throw error;
  }
}
...
export default function CardBeli({
  ...
  const [jasa, setJasa] = useState<string>("");
  const [hargaOngkir, setHargaOngkir] = useState<number>(0);
