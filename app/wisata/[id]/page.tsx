    ...
    <div>
      <div className="flex flex-col items-center justify-center">
        ...
        <h1 className="text-black text-[40px] font-semibold text-center">
          Lokawisata {imageById[0].nama}
        </h1>

        {/* Images */}
        <div className="flex flex-col md:flex-row w-[90%] mt-8 gap-5">
          <Image
            src={imageById[0].src.image1}
            alt={imageById[0].nama}
            width={737}
            height={395}
        ...
        <div className="w-[90%] mt-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 md:gap-5">
            <Link href={`/wisata/${imageById[0].hotelTerdekat[0].id}`}>
              <div className="flex-1">
                <Image
                  src={imageById[0].hotelTerdekat[0].src}
                  alt={imageById[0].hotelTerdekat[0].nama}
                  width={659}
                  height={390}
                  className="rounded-xl relative z-0 shadow-sm"
                />
                <p className="relative bottom-16 left-10 text-white text-[25px] font-bold">
                  {imageById[0].hotelTerdekat[0].nama}
                </p>
              </div>
            </Link>

            <Link href={`/wisata/${imageById[0].hotelTerdekat[1].id}`}>
              <div className="flex-1">
                <Image
                  src={imageById[0].hotelTerdekat[1].src}
                  alt={imageById[0].hotelTerdekat[1].nama}
                  width={659}
                  height={390}
                  className="rounded-xl relative z-0 shadow-sm"
                />
                <p className="relative bottom-16 left-10 text-white text-[25px] font-bold">
                  {imageById[0].hotelTerdekat[1].nama}
                </p>
              </div>
            </Link>
          </div>
